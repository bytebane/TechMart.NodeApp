...
exports.signUp = async (req, res, next) => {
  ...
  try {
    ...
    return res
      ...
      .send({
        success: true,
        message: "Admin A/C created successfully!",
        "auth-token": token,
    ...
    return res
      ...
      .send({
        success: false,
        message: "Admin A/C  Creation Failed!",
        error: err,
      });
  }
};
...
exports.logIn = async (req, res) => {
  ...
  try {
    ...
    return res
      ...
      .send({ success: true, "auth-token": token, userId: foundAdmin._id });
  } catch (error) {
...
exports.updateAdmin = async (req, res) => {
  try {
    ...
    if (emailExist) {
      return res
        ...
        .send({
          success: false,
          message:
            "A User with the same Email already exists! Try another one.",
        });
    }
    ...
    if (!updatedAdmin) {
      return res
        .status(400)
        .send({ success: false, message: "Could not update. Retry." });
    }
    return res
      .status(200)
      .send({ success: true, message: "Successfully updated", updatedAdmin });
  } catch (error) {
    return res
      .status(400)
      .send({
        success: false,
        message: "An error has occured, unable to update",
        error: error,
      });
  }
};
...
exports.deleteAdmin = async (req, res) => {
  try {
    ...
    }); // the `await` is very important here!

    if (!deletedAdmin) {
      return res
        .status(400)
        .send({ success: false, message: "Could not delete admin" });
    }
    return res
      .status(200)
      .send({
    ...
    return res
      ...
      .send({
        success: false,
        message: "An error has occurred, unable to delete Admin",
        error: error,
      });
  }
};
